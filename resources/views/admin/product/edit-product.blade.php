        ...
        <div class="col-md-12">
            <div class="cr-card card-default">
                <div class="cr-card-content">
                    <form action="{{ route('product.update', $show->product_id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                                ...
                                <div class="cr-vendor-upload-detail">
                                    <div class="row g-3">
                                                ...
                                                @foreach($category as $item)
                                                    <optgroup label="{{ $item->name }}">
                                                        @foreach($item->children as $childrens)
                                                            <option value="{{ $childrens->category_id }}" 
                                                                @if($childrens->category_id == $show->category_id) selected @endif>
                                                                {{ $childrens->category_name }}
                                                            </option>
                                                        @endforeach
                                                    </optgroup>
                                                @endforeach
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <textarea name="product_desc" id="editor1" cols="80" rows="70">{!! $show->product_desc !!}</textarea>
                                        </div>
                                        <div class="col-md-12">
